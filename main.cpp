...
void FormatSpeedC(unsigned long long bytes, wchar_t* outBuf, size_t bufSize) {
    ...
    if (unitIndex == 0) _snwprintf(outBuf, bufSize, L"%.0f %s", speed, units[unitIndex]);
    else if (speed >= 100) _snwprintf(outBuf, bufSize, L"%.0f %s", speed, units[unitIndex]);
    else _snwprintf(outBuf, bufSize, L"%.1f %s", speed, units[unitIndex]);
}
...
void FillRectLogical(DWORD* pixels, int lx, int ly, int lw, int lh, int iconSize, DWORD color) {
    ...
    if (xEnd > iconSize) xEnd = iconSize;
    if (yEnd > iconSize) yEnd = iconSize;
...
void DrawDigit(DWORD* pixels, int startX, int digit, int iconSize) {
    if (digit < 0 || digit > 9) return;
    for (int ly = 0; ly < 12; ++ly) {
        unsigned char row = font4x12[digit][ly];
        for (int lx = 0; lx < 4; ++lx) {
            if (row & (1 << (3 - lx))) {
                FillRectLogical(pixels, startX + lx, ly + 4, 1, 1, iconSize, 0xFFFFFFFF);
            }
        }
    }
}
...
HICON CreateSpeedIconC(unsigned long long speedBytes, bool isDownload) {
    ...
    if (unitIndex == 0) { // B
        ...
        if (unitIndex >= 1) FillRectLogical(pixels, 0, 0, 4, 3, iconSize, unitColor);
        if (unitIndex >= 2) FillRectLogical(pixels, 5, 0, 4, 3, iconSize, unitColor);
        if (unitIndex >= 3) FillRectLogical(pixels, 10, 0, 4, 3, iconSize, unitColor);
    }
    ...
    if (showD1) DrawDigit(pixels, 0, d1, iconSize);
    if (showD2) DrawDigit(pixels, 6, d2, iconSize);
    if (showD3) DrawDigit(pixels, 12, d3, iconSize);
        ...
        if (pIfTable) {
            if (GetIfTable(pIfTable, &dwSize, FALSE) == NO_ERROR) {
                for (DWORD i = 0; i < pIfTable->dwNumEntries; i++) {
                    ...
                    if (row.dwOperStatus == MIB_IF_OPER_STATUS_OPERATIONAL && row.dwType != 24) { // 24 is loopback
                        // Note: Virtual/tunnel adapters (VPNs, WSL, Hyper-V) are also included here and may double-count physical traffic.
                        if (calcDown) tempDown += (unsigned long long)row.dwInOctets;
                        if (calcUp) tempUp += (unsigned long long)row.dwOutOctets;
                    }
                }
            }
            free(pIfTable);
...
void LoadOrAskSettings() {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\InternetSpeedMonitor", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        if (RegQueryValueExW(hKey, L"Configured", NULL, &type, (BYTE*)&val, &size) == ERROR_SUCCESS && val == 1) {
            configured = true;
            size = sizeof(DWORD);
            if (RegQueryValueExW(hKey, L"ShowDown", NULL, &type, (BYTE*)&val, &size) == ERROR_SUCCESS) showDownIcon = (val != 0);
            size = sizeof(DWORD);
            if (RegQueryValueExW(hKey, L"ShowUp", NULL, &type, (BYTE*)&val, &size) == ERROR_SUCCESS) showUpIcon = (val != 0);
        }
        RegCloseKey(hKey);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_TIMER:
            if (wParam == ID_TIMER_UPDATE) {
                ...
                if (showDownIcon) {
                    ...
                    if (dBytes > peakDown) peakDown = dBytes;
                ...
                if (showUpIcon) {
                    ...
                    if (uBytes > peakUp) peakUp = uBytes;
                ...
                if (showDownIcon) prevDown = currDown;
                if (showUpIcon) prevUp = currUp;
            }
            break;
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == ID_TRAY_EXIT + 1) { // Exit Down
                showDownIcon = false;
                Shell_NotifyIconW(NIM_DELETE, &nidDown);
                if (nidDown.hIcon) DestroyIcon(nidDown.hIcon);
                nidDown.hIcon = NULL;
                if (!showDownIcon && !showUpIcon) PostQuitMessage(0);
            }
            else if (LOWORD(wParam) == ID_TRAY_EXIT + 2) { // Exit Up
                showUpIcon = false;
                Shell_NotifyIconW(NIM_DELETE, &nidUp);
                if (nidUp.hIcon) DestroyIcon(nidUp.hIcon);
                nidUp.hIcon = NULL;
                if (!showDownIcon && !showUpIcon) PostQuitMessage(0);
            }
            break;
